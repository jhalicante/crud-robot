...
export function UpdateRobotModal(props: {
  toggleModal: () => void;
  loadData: () => void;
  robotId: string;
  robots: Array<RobotInfoData>;
  showModal: boolean;
}) {
  ...
  useEffect(() => {
    const collaboratorMembers = (selectedRobotInfo()?.collaboratorMembers.map((member) => member.robotId) || []) as any;
    createRobotInfoFormik.setFieldValue('fullName', selectedRobotInfo()?.robotInfo.fullName || '');
    createRobotInfoFormik.setFieldValue('position', selectedRobotInfo()?.robotInfo.position || '');
    createRobotInfoFormik.setFieldValue('assignedProject', selectedRobotInfo()?.robotInfo.assignedProject || '');
    createRobotInfoFormik.setFieldValue('collaboratorMembers', collaboratorMembers);
  }, []);

  const onClickSubmit = (): void => {
    createRobotInfoFormik.submitForm();
    PATCH(`/robot-info/${props.robotId}`, createRobotInfoFormik.values).then((res: APIResponse) => {
      if (res.success) {
        props.loadData();
        props.toggleModal();
      } else {
  ...
  const selectedRobotInfo = (): RobotInfoData | null => {
    const roboInfo = props.robots.find((robot: RobotInfoData) => robot.robotInfo.robotId === props.robotId);
    if (roboInfo) {
      return roboInfo;
    }
    return null;
        ...
        <div className="fixed top-0 left-0 flex items-center justify-center w-full h-screen antialiased transition-all">
          ...
          <div className="relative z-10 flex flex-col w-11/12 max-w-2xl mx-auto border border-gray-300 rounded-lg shadow-xl sm:w-5/6 lg:w-1/2">
            <div className="flex flex-row items-center justify-between px-6 py-3 bg-white border-b border-gray-200 rounded-tl-lg rounded-tr-lg">
              <p className="text-lg font-semibold text-gray-600">{selectedRobotInfo()?.robotInfo.fullName} - Information</p>
              <button
                ...
                <div className="relative flex flex-col space-y-2">
                  ...
                  <div className="flex flex-wrap">
                    {props.robots.map((value: RobotInfoData, key) => {
                      return (
                        <>
                          {value.robotInfo.robotId !== props.robotId && (
                            <button
                              key={value.robotInfo.robotId}
...
export default UpdateRobotModal;
